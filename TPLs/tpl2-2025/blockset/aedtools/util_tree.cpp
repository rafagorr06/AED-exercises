...
namespace aed {
  ...
  //---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>
  // Fill the tree with random ids in range [0,nnodes)
  void fill_ids(tree<int> &T,tree<int>::iterator n,vector<int> &ids,int &pos) {
    if (n==T.end()) return;
    *n = ids[pos++];
    auto c = n.lchild();
    while (c!=T.end()) {
      fill_ids(T,c,ids,pos);
      c++;
    }
  }
  
  //---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>
  // Fill the tree with random ids in range [0,nnodes) (wrapper)
  void fill_ids(tree<int> &T) {
    int n = T.size();
    vector<int> ids(n);
    for (int k=0; k<n; k++) ids[k]=k;
    random_shuffle(ids.begin(),ids.end());
    if (T.empty()) return;
    int pos=0;
    fill_ids(T,T.begin(),ids,pos);
  }
  ...
  void apply(tree<int> &T,tree<int>::iterator n,scalarfunc_t &f) {
    if (n==T.end()) return;
    *n = f.map(*n);
    auto c = n.lchild();
  ...
  // Wrapper
  void apply(tree<int> &T,scalarfunc_t &f) {
    apply(T,T.begin(),f);
  }
